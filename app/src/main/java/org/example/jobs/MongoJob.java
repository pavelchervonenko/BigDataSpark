...
public class MongoJob {
    public static void run() {
        SparkSession spark = SparkSession.builder()
                .appName("MongoReportJob")
                .config("spark.mongodb.write.connection.uri", AppConfig.MONGO_URI)
                .config("spark.mongodb.read.connection.uri", AppConfig.MONGO_URI)
                .getOrCreate();
        ...
        // ЗАПИСЬ В MongoDB
        writeToMongo(reportProducts, "report_products");
        writeToMongo(reportCustomers, "report_customers");
        writeToMongo(reportTime, "report_time");
        writeToMongo(reportStores, "report_stores");
        writeToMongo(reportSuppliers, "report_suppliers");
        writeToMongo(reportQuality, "report_quality");

        spark.stop();
    }

    private static void writeToMongo(Dataset<Row> df, String collection) {
        df.write()
                .format("mongodb")
                .option("database", AppConfig.MONGO_DATABASE)
